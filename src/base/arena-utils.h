#ifndef NYAA_BASE_ARENA_UTILS_H_
#define NYAA_BASE_ARENA_UTILS_H_
...
public:
    ...
    static ArenaString *New(base::Arena *arena, const char *s) { return New(arena, s, strlen(s)); }

    static ArenaString *New(base::Arena *arena, std::string_view s) { return New(arena, s.data(), s.size()); }
...
public:
    ...
    explicit ArenaAllocator(const ArenaAllocator &other) throw() : ArenaAllocator<T>(other.arena_) {}
    template <typename U>
    ArenaAllocator(const ArenaAllocator<U> &other) throw() : ArenaAllocator<T>(other.arena()) {}
    ...
    T *allocate(size_t n, const void *hint = 0) { return arena_->NewArray<T>(n); }
    ...
    bool operator==(ArenaAllocator const &other) const { return arena_ == other.arena_; }
    bool operator!=(ArenaAllocator const &other) const { return arena_ != other.arena_; }
...
template <class T>
struct ArenaLess : public std::binary_function<T, T, bool> {
    bool operator()(const T &lhs, const T &rhs) const { return lhs < rhs; }
};  // struct ArenaLess

template <>
struct ArenaLess<ArenaString *> : public std::binary_function<ArenaString *, ArenaString *, bool> {
    bool operator()(ArenaString *lhs, ArenaString *rhs) const { return ::strcmp(lhs->data(), rhs->data()) < 0; }
};  // template<> struct ArenaLess<ArenaString *>

template <>
struct ArenaLess<const ArenaString *> : public std::binary_function<const ArenaString *, const ArenaString *, bool> {
...
template <class T>
struct ArenaHash : public std::unary_function<T, size_t> {
    size_t operator()(T value) const { return std::hash<T>{}(value); }
};  // struct ZoneHash

template <>
struct ArenaHash<ArenaString *> : public std::unary_function<ArenaString *, size_t> {
    size_t operator()(ArenaString *value) const { return value->hash_val(); }
};  // template <> struct ArenaHash<ArenaString *>

template <>
struct ArenaHash<const ArenaString *> : public std::unary_function<const ArenaString *, size_t> {
    size_t operator()(const ArenaString *value) const { return value->hash_val(); }
};  // template <> struct ArenaHash<const ArenaString *>

template <class T>
struct ArenaEqualTo : public std::binary_function<T, T, bool> {
    bool operator()(T lhs, T rhs) { return std::equal_to<T>{}(lhs, rhs); }
};  // template <class T> struct ArenaEqualTo

template <>
struct ArenaEqualTo<ArenaString *> : public std::binary_function<ArenaString *, ArenaString *, bool> {
...
template <>
struct ArenaEqualTo<const ArenaString *> : public std::binary_function<const ArenaString *, const ArenaString *, bool> {
    bool operator()(const ArenaString *lhs, const ArenaString *rhs) const {
        return lhs->size() == rhs->size() && ::memcmp(lhs->data(), rhs->data(), lhs->size()) == 0;
    }
...
public:
    ...
    explicit ArenaVector(Arena *zone) : std::vector<T, ArenaAllocator<T>>(ArenaAllocator<T>(zone)) {}

    // Constructs a new vector and fills it with {size} elements, each
    // constructed via the default constructor.
    ArenaVector(size_t size, Arena *zone) : std::vector<T, ArenaAllocator<T>>(size, T(), ArenaAllocator<T>(zone)) {}
...
public:
    // Constructs an empty vector.
    explicit ArenaDeque(Arena *zone) : std::deque<T, ArenaAllocator<T>>(ArenaAllocator<T>(zone)) {}

    // Constructs a new vector and fills it with {size} elements, each
    // constructed via the default constructor.
    ArenaDeque(size_t size, Arena *zone) : std::deque<T, ArenaAllocator<T>>(size, T(), ArenaAllocator<T>(zone)) {}
...
class ArenaMap : public std::map<K, V, Compare, ArenaAllocator<std::pair<const K, V>>> {
public:
    // Constructs an empty map.
    explicit ArenaMap(Arena *zone)
        : std::map<K, V, Compare, ArenaAllocator<std::pair<const K, V>>>(Compare(),
...
//==================================================================================================
template <class K, class V, class Hash = ArenaHash<K>, class KeyEqual = std::equal_to<K>>
class ArenaUnorderedMap : public std::unordered_map<K, V, Hash, KeyEqual, ArenaAllocator<std::pair<const K, V>>> {
public:
    // Constructs an empty map.
    explicit ArenaUnorderedMap(Arena *zone)
        : std::unordered_map<K, V, Hash, KeyEqual, ArenaAllocator<std::pair<const K, V>>>(
              100, Hash(), KeyEqual(), ArenaAllocator<std::pair<const K, V>>(zone)) {}
};

struct ArenaUtils {

    template <class T>
    static inline bool IsEmpty(const ArenaVector<T> *a) {
        return !a || a->empty();
    }

    template <class T>
    static inline bool IsNotEmpty(const ArenaVector<T> *a) {
        return !IsEmpty(a);
    }
};
...
#endif  // NYAA_BASE_ARENA_UTILS_H_
