#ifndef NYAA_BASE_IO_UTILS_H_
#define NYAA_BASE_IO_UTILS_H_
...
class FileReader {
public:
    FileReader(SequentialFile *file, bool ownership = false) : file_(DCHECK_NOTNULL(file)), ownership_(ownership) {}
    ...
    void Skip(size_t n) {
        error_ = file_->Skip(n);
        if (error_.ok()) { read_size_ += n; }
    }

    static Error ReadAll(const std::string &file_name, std::string *result, Env *env) {
...
public:
    RandomAccessFileReader(RandomAccessFile *file, bool ownership = false)
        : file_(DCHECK_NOTNULL(file)), ownership_(ownership) {}
    ...
    std::string_view Read(uint64_t offset, size_t n) { return Read(offset, n, &scratch_); }
    uint8_t          ReadByte(uint64_t offset) {
        std::string_view result = Read(offset, 1, &scratch_);
        return result[0];
    }
    uint16_t ReadFixed16(uint64_t offset) { return Slice::SetFixed16(Read(offset, 2, &scratch_)); }
    uint32_t ReadFixed32(uint64_t offset) { return Slice::SetFixed32(Read(offset, 4, &scratch_)); }
    uint64_t ReadFixed64(uint64_t offset) { return Slice::SetFixed64(Read(offset, 8, &scratch_)); }
...
public:
    FileWriter(WritableFile *file, bool ownership = false) : file_(DCHECK_NOTNULL(file)), ownership_(ownership) {}
    ...
    Error Write(std::string_view buf) {
        ...
        if (rs.ok()) { written_position_ += buf.size(); }
        return rs;
    }

    Error WritePad(size_t n) { return Write(Slice::GetPad(n, &scope_)); }

    Error Write(const void *p, size_t n) { return Write(std::string_view(static_cast<const char *>(p), n)); }

    Error WriteByte(char b) { return Write(std::string_view(&b, 1)); }

    Error WriteFixed16(uint16_t value) { return Write(Slice::GetU16(value, &scope_)); }

    Error WriteFixed32(uint32_t value) { return Write(Slice::GetU32(value, &scope_)); }

    Error WriteFixed64(uint64_t value) { return Write(Slice::GetU64(value, &scope_)); }
    ...
    static Error WriteAll(const std::string &file_name, std::string_view data, Env *env) {
...
public:
    ...
    BufferedWritableFile(WritableFile *file, bool ownership) : file_(DCHECK_NOTNULL(file)), ownership_(ownership) {}

    virtual ~BufferedWritableFile() {
        if (!buf_.empty()) {
            Error rs = file_->Append(buf_);
            if (!rs) { LOG(ERROR) << "Flush file fail!"; }
        }
        if (ownership_) { delete file_; }
    }
    ...
    virtual Error Append(std::string_view data) override {
        ...
        if (data.size() >= kBufferSize) {
            rs = Flush();
            if (rs.ok()) { rs = file_->Append(data); }
        } else {
            if (buf_.size() + data.size() > kBufferSize) { rs = Flush(); }
            if (rs.ok()) { buf_.append(data); }
        }
        return rs;
    }
