#ifndef NYAA_BASE_REFERENCE_COUNT_H_
#define NYAA_BASE_REFERENCE_COUNT_H_
...
public:
    ...
    intrusive_ptr(T *naked) : naked_(naked) {
        if (naked_) { naked_->AddRef(); }
    }

    intrusive_ptr(const intrusive_ptr &other) : intrusive_ptr(other.naked_) {}

    intrusive_ptr(intrusive_ptr &&other) : naked_(other.naked_) { other.naked_ = nullptr; }
    ...
    template <class S>
    static intrusive_ptr Make(S *naked) {
        return intrusive_ptr<T>(naked);
    }
...
public:
    ...
    atomic_intrusive_ptr(T *naked) : naked_(naked) {
        if (naked) { naked->AddRef(); }
    }

    atomic_intrusive_ptr(const atomic_intrusive_ptr &other) { reset(other.naked_); }
    ...
    void reset(T *naked) {
        ...
        if (naked) { naked->AddRef(); }
        T *e;
        do { e = old_val; } while (naked_.compare_exchange_strong(e, naked));
    }
    ...
    static atomic_intrusive_ptr Null() { return atomic_intrusive_ptr<T>(nullptr); }
...
template <class T>
inline intrusive_ptr<T> MakeRef(T *naked) {
    return intrusive_ptr<T>(naked);
}
...
public:
    ...
    void AddRef() const { ref_count_.fetch_add(1, std::memory_order_acq_rel); }
...
public:
    ...
    void ReleaseRef() const {
        DCHECK_GT(ref_count(), 0);
        if (ref_count_.fetch_sub(1, std::memory_order_acq_rel) == 1) { delete static_cast<const T *>(this); }
    }
...
#endif  // NYAA_BASE_REFERENCE_COUNT_H_
