...
static inline T *Align(T *x, size_t alignment) {
    return reinterpret_cast<T *>(RoundUp(reinterpret_cast<uintptr_t>(x), alignment));
}
...
public:
    ...
    size_t cache_size() const { return cache_size_.load(std::memory_order_acquire); }
    ...
    static_assert(sizeof(Zone::Page) == sizeof(Zone::ShadowPage), "Page and shadow not has same size.");
...
public:
    ...
    ArenaImpl(Zone::Core *owns, size_t limit_size) : owns_(owns), limit_size_(limit_size), memory_usage_(0) {}

    virtual ~ArenaImpl() {
        Purge(false);
        if (this != owns_->dummy()) { owns_->RemoveArena(this); }
    }
...
void *Zone::ArenaImpl::AllocateSmall(size_t size, size_t alignment) {
    ...
    if (!current_) {
        ...
        if (current_->free + size > end) { need_new_page = true; }
    }
...
/*virtual*/ void *Zone::Allocate(size_t size, size_t alignment) { return core_->dummy()->Allocate(size, alignment); }
